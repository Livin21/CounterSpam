...
class Classifier {
    ...
    private fun conditionalWord(word: String, spam: Boolean, numWords: Int): Double? {
        // Laplace smoothing
        val smoothingFactor = if (trainPositive[word] == null) 1 else 0
        val trainPositiveVal = if (trainPositive[word] == null) 0 else trainPositive[word]
        val trainNegativeVal = if (trainNegative[word] == null) 0 else trainNegative[word]
        return if (spam)
            (trainPositiveVal!! + smoothingFactor) / (positiveTotal + smoothingFactor * numWords).toDouble()
        else
            (trainNegativeVal!! + smoothingFactor) / (negativeTotal + smoothingFactor * numWords).toDouble()
    }

    private fun conditionalSms(body: String, spam: Boolean): Double {
        var result = 1.0
        for (word in body.split(" "))
            result *= conditionalWord(word, spam, body.split(" ").size) ?: 1.0
        return result
    }

    private fun classify(sms: SMS): Boolean {
    ...
    fun updateDataSet(context: Context, sms: SMS) {
        ...
        if (sms.label == "spam") {
            try {
                ...
                spamData += "${sms.source} $delimiter ${sms.body.replace("\n"," ")}"
